...
const tprow titable[] = {{30, 400, -9.0}, {40, 310, -9.0}, {50, 240, -7.0},
                         {60, 175, -6.5}, {70, 138, -3.7}, {80, 100, -3.8},
                         {90, 80, -2.0},  {95, 70, -2.0},  {140, 0, -1.6}};
...
class EngineTemp {
 ...
 public:
  ...
  float fromadc() {
    ...
    const int n = 10;
    for (int i = 0; i < n; i++) {
      adc += analogRead(e_temp_in);
    }
    adc = adc / n;
    int rows = sizeof(titable) / sizeof(tprow);
    for (int i = 0; i < rows; i++) {
      tprow r = titable[i];
      if (adc >= r.p) {
        return r.t + (adc - r.p) / r.d;
      }
    }
    return 140;
  }
